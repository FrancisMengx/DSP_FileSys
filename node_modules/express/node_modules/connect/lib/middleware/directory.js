...
function html(files, dir, useIcons) {
  return '<ul id="files">' + files.map(function(file){
    ...
    if (useIcons && '..' != file) {
      ...
      icon = '<img src="data:image/img;base64,' + load(icon) + '" />';
      classes.push('icon');
    }
...
var icons = {
    '.js': 'page_white_code_red.img'
  , '.c': 'page_white_c.img'
  , '.h': 'page_white_h.img'
  , '.cc': 'page_white_cplusplus.img'
  , '.php': 'page_white_php.img'
  , '.rb': 'page_white_ruby.img'
  , '.cpp': 'page_white_cplusplus.img'
  , '.swf': 'page_white_flash.img'
  , '.pdf': 'page_white_acrobat.img'
  , 'default': 'page_white.img'
};
